...
try {
    $pdo->exec("ALTER TABLE `cash_book` MODIFY COLUMN `payment_method` VARCHAR(50) NOT NULL DEFAULT 'cash'");
} catch (Exception $e) {
}
try {
    $pdo->exec("ALTER TABLE `cash_book` DROP FOREIGN KEY `cash_book_ibfk_3`");
} catch (Exception $e) {
}
...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] === 'pay' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$cashAccountId) {
            echo json_encode(['success' => false, 'message' => 'Rekening tujuan wajib dipilih']);
            exit;
        }
        ...
        if (!$accRow) {
            echo json_encode(['success' => false, 'message' => 'Rekening tidak valid']);
            exit;
        }
        $isCashType = $accRow['account_type'] === 'cash';
        if (($paymentMethod === 'cash' && !$isCashType) || ($paymentMethod !== 'cash' && $isCashType)) {
            echo json_encode(['success' => false, 'message' => 'Rekening tidak sesuai dengan metode pembayaran yang dipilih']);
            exit;
        }
        try {
            $db->beginTransaction();
            ...
            if ($cashAccountId) {
                try {
                    ...
                    error_log("Cash account update: " . $e->getMessage());
                }
            }
            $db->commit();
            ...
            if ($inv['status'] === 'paid') {
                ...
                if (!empty($inv['cash_account_id'])) {
                    try {
                        ...
                        error_log("Cash account reverse: " . $e->getMessage());
                    }
                }
            }
    ...
    if ($_GET['ajax'] === 'upload_logo' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        if (empty($_FILES['logo']) || $_FILES['logo']['error'] !== UPLOAD_ERR_OK) {
            echo json_encode(['success' => false, 'message' => 'File logo tidak valid']);
            exit;
        }
        $fileExt = strtolower(pathinfo($_FILES['logo']['name'], PATHINFO_EXTENSION));
        if (!in_array($fileExt, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
            echo json_encode(['success' => false, 'message' => 'Format file tidak didukung (jpg/png/gif/webp)']);
            exit;
        }
        try {
            ...
            $result = $cloudinary->smartUpload(
                $_FILES['logo'],
                'uploads/logos',
                ACTIVE_BUSINESS_ID . '_logo.' . $fileExt,
                'logos',
                'company_logo_' . ACTIVE_BUSINESS_ID
            );
            if (!$result['success']) {
                echo json_encode(['success' => false, 'message' => $result['error'] ?? 'Upload gagal']);
                exit;
            }
            $stmt = $pdo->prepare("INSERT INTO settings (setting_key, setting_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value)");
    ...
    if ($_GET['ajax'] === 'upload_qr' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        if (empty($_FILES['qr']) || $_FILES['qr']['error'] !== UPLOAD_ERR_OK) {
            echo json_encode(['success' => false, 'message' => 'File QR tidak valid']);
            exit;
        }
        ...
        try {
            ...
            $result = $cloudinary->smartUpload(
                $_FILES['qr'],
                'uploads/qr',
                ACTIVE_BUSINESS_ID . '_invoice_qr.' . $fileExt,
                'invoice-qr',
                'invoice_qr_' . ACTIVE_BUSINESS_ID
            );
            if (!$result['success']) {
                echo json_encode(['success' => false, 'message' => $result['error'] ?? 'Upload gagal']);
                exit;
...
if ($search) {
    $where .= " AND (ci.invoice_number LIKE ? OR ci.customer_name LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
}
...
<style>
    ...
    .inv-filters a.active {
        background: #fff;
        color: var(--cafe);
        box-shadow: var(--shadow-sm);
    }

    .inv-filters a:hover:not(.active) {
        color: var(--cafe-dark);
    }
    ...
    .btn-create:hover {
        box-shadow: 0 6px 24px rgba(3, 105, 161, .4);
    }
    ...
    .item-row {
        display: grid;
        grid-template-columns: 1fr 60px 100px 90px 30px;
        gap: 6px;
        align-items: end;
        margin-bottom: 5px;
        padding: 5px 7px;
    ...
    .totals-box {
        background: linear-gradient(135deg, var(--cafe-cream), #f0f9ff);
    ...
    .modal-box {
        ...
        max-height: 90vh;
        overflow-y: auto;
        box-shadow: 0 20px 60px rgba(3, 105, 161, .15);
    ...
    .inv-items-tbl thead th:first-child {
        border-radius: 6px 0 0 0;
    }

    .inv-items-tbl thead th:last-child {
        border-radius: 0 6px 0 0;
        text-align: right;
    }

    .inv-items-tbl tbody td {
        padding: 7px 10px;
        border-bottom: 1px solid #e0f2fe;
    }

    .inv-items-tbl tbody tr:nth-child(even) {
        background: #f0f9ff;
    }
    ...
    .inv-items-tbl td:nth-child(3) {
        text-align: center;
    }

    .inv-items-tbl td:nth-child(4) {
        text-align: right;
    }
    ...
    .inv-bank-box {
        ...
        padding: 9px 12px;
        background: var(--cafe-cream);
        border: 1.5px dashed var(--cafe-gold);
        border-radius: 8px;
    }
    ...
    .inv-status-line.paid {
        color: #059669;
    }

    .inv-status-line.unpaid {
        color: #dc2626;
    }
    ...
    .inv-footer-bar .tagline {
        font-size: 9.5px;
        color: #9ca3af;
        font-style: italic;
    }

    .inv-footer-bar .legal {
        font-size: 8.5px;
        color: #d1d5db;
        margin-top: 4px;
    }
    ...
    @media (max-width: 768px) {
        ...
        .item-row,
        .item-header {
            grid-template-columns: 1fr 60px 100px 36px;
        }

        .item-row .line-total-col,
        .item-header .line-total-col {
            display: none;
        }
...
<div id="viewList">
    ...
    <div class="inv-stats">
        <div class="inv-stat s1">
            <h4>TOTAL INVOICE</h4>
            <p class="val"><?php echo (int)$stats['total']; ?></p>
        </div>
        <div class="inv-stat s2">
            <h4>BELUM BAYAR</h4>
            <p class="val"><?php echo (int)$stats['unpaid_count']; ?></p>
            <div class="sub"><?php echo formatCurrency($stats['unpaid_amount'] ?? 0); ?></div>
        </div>
        <div class="inv-stat s3">
            <h4>LUNAS HARI INI</h4>
            <p class="val"><?php echo formatCurrency($stats['today_paid'] ?? 0); ?></p>
        </div>
        <div class="inv-stat s4">
            <h4>TOTAL LUNAS</h4>
            <p class="val"><?php echo (int)$stats['paid_count']; ?></p>
        </div>
    </div>
    ...
    <form method="POST" id="createForm">
        ...
        <div class="cf-card">
            ...
            <div class="item-header">
                ...
                <div>Qty</div>
                <div>Harga</div>
                <div class="line-total-col">Subtotal</div>
                <div></div>
            </div>
...
<script>
    ...
    function showCreate() {
        ...
        document.getElementById('viewCreate').style.display = 'block';
    }
        ...
        fetch('?ajax=upload_qr', {
            ...
            .then(function(data) {
                ...
                if (data.success) {
                    ...
                    alert(data.message || 'Gagal upload QR');
                }
            }).catch(function() {
                btn.disabled = false;
                btn.textContent = 'Upload QR';
    ...
    function openPayModal(id, num, amount) {
        ...
        document.querySelectorAll('#payAccount option[data-type]').forEach(function(o) {
            o.hidden = false;
            o.disabled = false;
        });
